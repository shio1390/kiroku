...
- キャッシュ名は `kiroku-pwa-v6-crop-date` です。更新時は新しいキャッシュを作り、古いキャッシュを削除します。
...
2. [src/image-cropper.js](src/image-cropper.js) で読み取り範囲のトリミングと90度回転を行う。
3. [src/ocr.js](src/ocr.js) が長辺2,200px以下へ縮小し、Tesseract.jsのWorkerで端末内OCRを実行する。
4. [src/import-parser.js](src/import-parser.js) がテキスト分割、日付、金額、タイトル候補を抽出する。
5. 全文と生成予定の記録を編集・選択してから、一括保存する。
6. [src/db.js](src/db.js) がアプリ状態と任意のOCR使用画像を1つのIndexedDBトランザクションで保存する。
...
- 「OCRに使った画像も保存する」は初期値オフです。オンにした場合だけ、トリミング後または画像全体のBlobを `assets` ストアへ保存します。

日付読み取り:

- `YYYY/MM/DD`、`YYYY-MM-DD`、`YYYY.MM.DD`、`YYYY年M月D日` に対応します。
- `YY/MM/DD`、`YY-MM-DD`、`YY.MM.DD`、`YY年M月D日` に対応します。
- 全角数字・全角区切り・区切り前後の空白を正規化してから判定します。
- 2桁年は `00`～`69` を2000年代、`70`～`99` を1900年代として解釈します。
- 月日だけの表記は、誤認識を避けるため `M月D日` のみ当年として扱います。
